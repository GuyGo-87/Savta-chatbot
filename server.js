...
- If unsure, say "Ach, I'm not sure motek — email hello@savtasspices.com and Niv will sort it out!"
- End recipe answers with "B'teavon! 🍽️"
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    const reply = data.candidates?.[0]?.content?.parts?.[0]?.text
      || "Ach, something got stuck in my kitchen. Try again, motek!";

    res.json({ reply });
  } catch (err) {
    console.error("Chat error:", err);
    res.status(500).json({ reply: "Oy vavoy! Something went wrong. Try again in a moment, neshama! 😅" });
  }
});
...
body::before {
  ...
  inset: 0;
  background:
    radial-gradient(ellipse 800px 600px at 10% 20%, rgba(122,35,24,0.05) 0%, transparent 60%),
    radial-gradient(ellipse 600px 400px at 90% 80%, rgba(245,158,11,0.07) 0%, transparent 55%);
  pointer-events: none;
  z-index: 0;
}
...
#savta-bubble {
  ...
  box-shadow: 0 8px 32px rgba(122,35,24,0.5), 0 2px 8px rgba(0,0,0,0.15);
  cursor: pointer;
  display: flex;
  align-items: center;
  justify-content: center;
...
#savta-bubble:hover {
  transform: scale(1.1);
  box-shadow: 0 14px 48px rgba(122,35,24,0.6);
}
#savta-bubble .bubble-emoji {
  font-size: 30px;
  line-height: 1;
...
#savta-window {
  ...
  box-shadow:
    ...
    inset 0 0 0 1px rgba(122,35,24,0.10);
  display: flex;
  flex-direction: column;
  z-index: 9998;
  overflow: hidden;
  ...
  opacity: 0;
  pointer-events: none;
  transition: transform 0.32s cubic-bezier(.34,1.4,.64,1), opacity 0.24s ease;
...
/* Header */
.savta-header {
  background: linear-gradient(135deg, #7a2318 0%, #9b2e1c 50%, #c0392b 100%);
...
.msg {
  ...
  animation: msgIn 0.28s cubic-bezier(.34,1.4,.64,1) forwards;
}
@keyframes msgIn {
...
.msg.user .msg-bubble {
  background: linear-gradient(135deg, #7a2318, #a0341f);
  color: #fff;
...
.savta-input {
  ...
  resize: none; max-height: 100px; min-height: 42px;
  line-height: 1.5;
    ...
    <div class="install-card">
      ...
      <ol class="install-steps">
        <li>
          ...
          <span>In Shopify Admin → <code>Online Store</code> → <code>Themes</code> → <code>Edit code</code></span>
        </li>
        <li>
          <span class="step-num">2</span>
          <span>Open <code>theme.liquid</code> and find the closing <code>&lt;/body&gt;</code> tag</span>
        </li>
        <li>
          <span class="step-num">3</span>
          <span>Paste the embed snippet from this server just before <code>&lt;/body&gt;</code></span>
        </li>
        <li>
          <span class="step-num">4</span>
          <span>Save. Savta appears on every page instantly! 🎉</span>
        </li>
      </ol>
    </div>
  </div>
...
<div id="savta-window" role="dialog" aria-label="Chat with Savta Marsel">
  ...
  <div class="quick-prompts" id="quick-prompts">
    ...
    <button class="quick-btn" onclick="sendQuick('Tell me about Savta Marsel and the story behind this brand')">💛 Story</button>
  </div>
...
function showTyping() {
  const c = document.getElementById('savta-messages');
  const el = document.createElement('div');
  el.className = 'msg bot'; el.id = 'savta-typing';
  el.innerHTML = \`
    <div class="msg-mini-avatar">👵</div>
...
async function sendMessage() {
  ...
  try {
    ...
    removeTyping();
    addBotMsg(data.reply || "Ach, something went wrong. Try again, motek!");
    history.push({ role: 'assistant', content: data.reply });
  } catch(e) {
    removeTyping();
    addBotMsg("Oy vavoy! Network problem. Try again in a moment, neshama! 😅");
  }
